...
public class DeleteProfileCommand extends CommandManager {
    ...
    public void processCommand() {
        if(sure) {
            ...
            try {
                profile.delete();
                Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN + "Successfully deleted profile. Now, restart minecraft to reset all settings in the mod."));
                GetterStats.init();
                GetterStats.reload();
            }
            catch (Exception ex) {
